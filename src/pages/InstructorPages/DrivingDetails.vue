<template>
  <div>
    <div class="flex column">
      ...
      <span class="q-ml-md q-mt-md q-mb-xs" :class="$style.lesson_studentname"
      >Константиненко Константин</span>
      <span class="q-mb-lg"  :class="$style.lesson_title"
        >{{lesson.date}}, {{lesson.time}}, вождение №4</span
      >

      <div class="q-mr-md q-ml-md"
           :class="$style.body_buttons__wrap">
        <span v-if="isStarted"
              :class="$style.body_time">Старт в {{`${this.dateTimeStarted.getHours()}:${this.dateTimeStarted.getMinutes()}`}}, до {{calcFinishTime()}}</span>
        <UiButton
          v-if="isStarted"
...
export default {
  ...
  created() {
  this.lesson = this.$store.getters["instructorInfo/getLessonById"](this.id);
  },
  data() {
    return {
      isStarted: false,
