...
>
  <div>
    <ui-page-title
      :class="$style.title"
      @backward="$router.push({ name: 'drivingDetails', params: {lesson: lesson, date: date}})"
      title="Курсант"
    >
    </ui-page-title>
    <div
      ...
      >
        ...
        >
          ...
          <span :class="$style.student_department">
            {{ studentInfo.branch }} <br />
            {{studentInfo.tarif}}<br />
           Договор №{{studentInfo.deal_id}}
          </span>
        </div>
      </div>
...
<script>
  ...
  export default {
    ...
    async created() {
      this.studentInfo = await this.$store.dispatch('instructorInfo/fetchStudentInfo', this.studentId);
      if (!this.studentInfo) {
        this.$router.push({ name: 'drivingDetails', params: {lesson: this.lesson, date: this.date}});
      }
    },
  };
</script>
