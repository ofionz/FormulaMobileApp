<template>
  <div
    :class="[modalMode?'':$style.container]"
    class="flex column"
  >
    <span
      v-if="!modalMode"
  ...
  export default {
    ...
    methods: {
      ...
      async saveButtonHandler() {
        ...
        if (await this.$store.dispatch('studentInfo/saveStudentProfile', this.data)) {
          ...
          this.$emit('saved');
          this.$route.name !== 'cabinet' ? this.$router.push({ name: 'cabinet' }) : '';
        }
